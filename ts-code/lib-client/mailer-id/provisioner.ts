...
export interface ProvisioningCompletion {
	...
	complete(
		dhsharedKeyCalc: ICalcDHSharedKey, certDuration: number, assertDuration?: number
	): Promise<MailerIdSigner>;
}
...
export class MailerIdProvisioner extends ServiceUser {
	...
	async provisionSigner(
		...
		const completion: ProvisioningCompletion['complete'] = async (
			...
			return makeMailerIdSigner(pair.skey, this.userCert, this.provCert, assertDuration);
		};
		return {
